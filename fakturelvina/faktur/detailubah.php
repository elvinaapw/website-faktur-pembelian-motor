...
$query = mysqli_query($conn, "SELECT * FROM faktur WHERE nofaktur = '$nofaktur'");
$data = mysqli_fetch_array($query);

// Query to get the previous values from the detail table
$detailQuery = mysqli_query($conn, "SELECT * FROM detail WHERE nofaktur = '$nofaktur'");
$detailData = mysqli_fetch_array($detailQuery);
?>
...
<body>
    <div class="faktur-ubah">
        <h3 class="text-center mt-5">FAKTUR DETAIL UBAH</h3>
        <form method="post" class="mt-5">
                ...
                <div class="col-sm-10">
                    <select class="form-select" id="BPKB" name="BPKB">
                        ...
                        while ($row = mysqli_fetch_array($query)) {
                            $selected = ($row['BPKB'] == $detailData['BPKB']) ? 'selected' : '';
                            echo "<option value='$row[BPKB]' $selected>$row[BPKB] - $row[warna] - $row[tipe]</option>";
                        }
                        ?>
                    </select>
                </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="number" class="form-control" id="unit" name="unit" value="<?php echo $detailData['unit']; ?>">
                </div>
            </div>
            <div class="row mt-5">
...
if (isset($_POST['proses'])) {
    ...
    $queryInsert = "UPDATE detail SET unit='$unit' WHERE nofaktur='$nofaktur' AND BPKB='$BPKB'";
